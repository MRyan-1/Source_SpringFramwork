...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	...
	public final BeanDefinition parse(Element element, ParserContext parserContext) {
		//look!parseInternal
		//虽说是对自定义配置文件的解析,但是可以看到,在这个函数中大部分的代码是用来处理将解析后的AbstractBeanDefinition转化为BeanDefinitionHolder并注册的功能,
		// 而真正去做解析的事情委托给了函数parseInternal,正是这句代码调用了我们自定义的解析函数。 在parseInternal中并不是直接调用自定义的doParse函数,而是进行了一系列的数据准备, 包括对beanClass, scope、 lazyLnit等属性的准备。
