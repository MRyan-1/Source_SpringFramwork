...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * autowire-candidate属性设置为false，这样容器在查找自动装配对象时，将不考虑该bean，即他不会被考虑作为其他bean自动装配的候选者，但是该bean本身还是可以使用自动装配来注入其他bean的
	 */
	...
	/**
	 * 定义这个bean的应用，APPLICATION：用户，INFRASTRUCTURE：完全内部使用，与用户无关，SUPPPORT:某些复杂配置的一部分 程序设置
	 */
	 ...
	 * 这两个功能实现原理其实是在bean实例化的时候如果检测到存在methodOverrides属性,会动态地为当前bean生成代理并使用对应的拦截器为bean做增强处理,相关逻辑实现在bean的实例化部分详细介绍。
	 *
