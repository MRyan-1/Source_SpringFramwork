...
public class ProxyCreatorSupport extends AdvisedSupport {
	...
	/**
	 * 首先使用了AopProxyFactory来常见AopProxy，AopProxyFactory使用的是DefaultAopProxyFactory，使用AopProxyFactory作为AopProxy的创建工厂对象
	 */
	protected final synchronized AopProxy createAopProxy() {
		if (!this.active) {
			activate();
		}
		//通过AopProxyFactory取得AopProxy，这个AopProxyFactory是在初始化函数中定义的 使用DefaultAopProxyFactory
